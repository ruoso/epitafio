...
use lib "$FindBin::Bin/../../Epitafio-DB/lib";

my $debug             = 0;
my $fork              = 0;
my $help              = 0;
my $host              = undef;
my $port              = $ENV{EPITAFIO_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{EPITAFIO_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
epitafio_server.pl - Catalyst Testserver

=head1 SYNOPSIS

epitafio_server.pl [options]
